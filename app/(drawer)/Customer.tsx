...
const CustomerListScreen = () => {
  ...
  const handleSearch = (text) => {
    setSearchText(text);
    debouncedFetchCustomers(text); // Use debounced fetch
  };

  useFocusEffect(
    useCallback(() => {
      fetchCustomers(); // Fetch customers without search query on screen focus
    }, [])
  );
  ...
  const renderItem = ({ item }) => (
    <TouchableOpacity
      style={styles.card}
      onPress={() => navigation.navigate('CustomerDetailsScreen', { customerId: item.id })}
    >
      ...
      <TouchableOpacity
        ...
        onPress={() => navigation.navigate('CreateCustomerScreen', { customerId: item.id })}
      >
