...
const CreatePurchasesScreen = () => {
  ...
  useEffect(() => {
    ...
    // Fetch purchases data in edit mode

    // Set the navigation title
    navigation.setOptions({
      headerTitle: purchaseId ? 'Edit Purchases' : 'Create Purchases',
    });
  ...
  const savePurchase = async () => {
    ...
    // if (!supplierId) {
    //   alert('Supplier ID not set. Please try again later.');
    //   return;
    // }
    // Prepare the purchase data to send
    const purchaseData = {
      purchaseDate,
      dueDate,
      tax: 0,
      discount: 0,
      supplierId: supplierId, // Assuming `selectedSupplier` is an object
    ...
    try {
      ...
      console.log('Purchase Data: Prem:  ', purchaseData); // Log the data to check values
      ...
      // console.error('Error occurred while saving purchase:', error.response?.data?.message || error.message);
      setErrorMessage(error.response?.data?.message || 'Error saving purchase. Please try again.');
    } finally {
  ...
  const handleRemoveItem = (id) => {
    ...
    setPurchaseItems(updatedItems); // Update the state
  };
  ...
  return (
    <SafeAreaView style={[styles.container, { paddingTop: headerHeight }]}>
      ...
      {/* Date Picker for Purchase Date and Due Date */}
      {/* <View style={{ flexDirection: 'row', justifyContent: 'space-between', margin: 5, marginTop: 0 }}>
        <View style={{ width: '49%' }}>
          <View style={{ margin: 8 }}>
            <Text>Purchase Date</Text>
            <Text style={{}}>{purchaseDate.toLocaleDateString()}</Text>
          </View>
...
export default CreatePurchasesScreen;
